...
class Executor:
    ...
    def replay_one_query(
        ...
        if scheduled_submit is not None:
            future = asyncio.ensure_future(submit_query_and_wait_for_result(self.db_conn, query_rep, query_sql))
    ...
    async def finish_all_queries(self,
        ...
        while len(self.scheduler.queued_queries) != 0 or len(self.scheduler.running_queries) != 0:
    ...
    def check_query_finished(self,
                             ...
                             new_runtime: MutableMapping[Union[int, str], float],
                             all_timeout: List[Union[int, str]],
                             all_error: List[Union[int, str]]) -> bool:
    ...
    async def replay_workload(self, directory: str) -> (
        ...
        for i in range(len(concurrency_df)):
            ...
            while current_time < current_query_start_time - 0.5:
                ...
                has_finished_queries = self.check_query_finished(current_time, new_runtime, all_timeout, all_error)
                if has_finished_queries:
            ...
            self.replay_one_query(
                current_time, i, all_query_sql[i], all_query_idx[i]
            )
        # finish all queries
        await self.finish_all_queries(function_start_time, new_runtime, all_timeout, all_error)
        ...
        return original_runtime, our_runtime, np.asarray(all_timeout), np.asarray(all_error)
