...
class TraceManager:
    ...
    def simulate_trace_one_iter(self, trace: pd.DataFrame) -> Tuple[pd.DataFrame, bool]:
        ...
        all_prediction, _ = self.predictor.predict(concurrency_df, return_per_query=False)
        ...
        for i in range(len(concurrency_df)):
            if ground_truth_runtime[i] <= 5:
                ...
                idx_sorted = int(np.where(query_rt_sort_idx == original_query_idx[i])[0])
                interval = np.searchsorted(self.avg_runtime_interval, all_prediction[i]) - 1
                if interval == 0:
                    ...
                    stride = 1
                if ground_truth_runtime[i] - all_prediction[i] > 10:
                    modified += 1
                    modified_minus += 1
                    query_idx = query_rt_sort_idx[min(idx_sorted + stride, len(query_rt_sort_idx) - 1)]
                    runtime = all_prediction[i] + np.abs(np.random.normal(all_prediction[i]/4, np.sqrt(all_prediction[i])))
                elif all_prediction[i] - ground_truth_runtime[i] > 10:
                    modified += 1
                    modified_plus += 1
                    query_idx = query_rt_sort_idx[max(idx_sorted - stride, 0)]
                    runtime = all_prediction[i] - min(np.abs(np.random.normal(all_prediction[i]/4, np.sqrt(all_prediction[i]))), all_prediction[i]/2)
                else:
        ...
        if self.debug:
            if self.logger is None:
                ...
                print(f"Modified plus {modified_plus} queries;  modified minus {modified_minus} queries")
            else:
                self.logger.info(f"Modified {modified} queries out of {len(trace)} queries")
                self.logger.info(f"Modified plus {modified_plus} queries;  modified minus {modified_minus} queries")
    ...
    def simulate_trace_iteratively(self,
                                trace_path: str,
        ...
        for i in range(len(trace)):
            if all_runtime[i] <= 5:
                ...
                interval = np.searchsorted(self.avg_runtime_interval, all_runtime[i]) - 1
