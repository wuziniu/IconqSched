...
class CachePredictor:
    def __init__(self, capacity=5000, alpha=1.0, hash_bits=None, store_all=False, use_index=False):
    ...
    def ingest_data(self, df):
        for i, rows in df.groupby("query_idx"):
            ...
            if hash_val not in self.running_average:
                ...
                self.most_recent[hash_val] = rows["runtime"][-1]
                ...
                self.running_average[hash_val] = (observed_rows * self.running_average[hash_val] + new_rows * np.average(rows["runtime"])) / new_rows
                self.most_recent[hash_val] = rows["runtime"][-1]
                # Todo: this is not the correct formula
                self.running_std[hash_val] = (observed_rows * self.running_std[hash_val] + new_rows * np.std(rows["runtime"])) / new_rows
    ...
    def predict(self, df):
        ...
        for i in range(len(df)):
            ...
            if hash_val not in self.running_average:
                ...
                pred = self.running_average[hash_val] * self.alpha + self.most_recent[hash_val] * (1 - self.alpha)
